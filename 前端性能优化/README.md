...
`Cache-Control`和`Expires`同时存在时，优先使用`Cache-Control`。`Cache-Control`是HTTP1.1	新增的字段，主要是为了解决`Expires`难以解决的问题。

* Expires是绝对时间，如果服务器和客户端时间偏差比较大，就回导致缓存混乱。比如服务器下发的时间是2021.3.1日后过期，但是用户把电脑当前时间调整到2019年，那么它的过期时间就会比真实的过期时间长的多。如果用户把电脑时间调整到2021.3.5，也就是服务器时间之后，那么这个资源会一直不被缓存，需要重新请求。所以`Expires`在使用中容易带来麻烦。因此，引入了`Cache-Control`，`Cache-Control`是一个相对时间，它通过max-age来判断。
...
我们可以发现强缓存过于简单粗暴了，只要本地有缓存就直接从本地拿，完全不管服务器有没有更新，这样可能导致拿到的始终不是最新的资源，因此一些资源就不能被缓存，比如 html 文件。因此，有没有不那么强势的缓存，会每次询问服务器是否有更新，然后根据是否服务器是否有更新来判断是否应该去去本地取缓存，这就是`协商缓存`。协商缓存是强制缓存失效以后，浏览器携带缓存标识向服务器发起请求，由服务器根据缓存标识决定是否使用缓存的过程,而不是像浏览器一样简单粗暴，只要本地有缓存就从本地拿数据。

协商缓存`ETag`和`Last-Modified`这两个字段进行判断。

**ETag:**是类似于md5的字符串，每个文件有一个作为标识，文件改动了，ETag也会发生变化。

**Last-Modified:**GMT格式的时间戳，用于标识当前文件的最新修改时间，如果两次时间不匹配，标识文件被修改了。

**Last-Modified/ETag**
![etag-last-modified](https://ftp.bmp.ovh/imgs/2021/03/725f0128fcda7dc2.jpg)

每次http返回回来的response header中都会带有`ETag`和`Last-Modified`，在下次请求时，request header中会把这两个带上（但是名字发生了变化`ETag` ->`If-None-Match`，`Last-Modified` -> `If-Modified-Since`）,服务端把你带过来的标识，资源目前的标识进行对比，然后判断资源是否更改了。

**协商缓存—服务器资源未改动**

![协商缓存—服务器资源未改动](https://ftp.bmp.ovh/imgs/2021/03/57b68557964e8012.png)



**协商缓存—服务器资源发生改动**
![协商缓存—服务器资源发生改动](https://ftp.bmp.ovh/imgs/2021/03/a181471d9e62568c.png)

所以协商缓存的步骤：

1. 请求资源时，把用户本地该资源的ETag同时带到服务器，服务器和最新的资源做对比
2. 如果资源没更改，返回304，浏览器直接从本地缓存读取资源
3. 如果资源有更改，返回最新的资源和200，浏览器将最新的ETag存储到本地缓存数据表中

#### 为什么要有Etag?

`ETag`和`Last-Modified`同时存在时，优先使用`ETag`。`ETag`是HTTP1.1中新增的字段，主要是为了解决`Last-modified`的比较难解决的问题：

* 一些文件也许会周期性的修改，但是内容并不改变只是更改了日期，这个时候我们不希望客户端认为这个文件被修改了，而重新请求。
* 某些文件修改非常频繁，比如在秒以下的时间内进行修改，(比方说1s内修改了N次)，`If-Modified-Since` 能检查到的粒度是秒级的，使用 `Etag` 就能够保证这种需求下客户端在1秒内能刷新 N 次 cache。
* 某些服务器不能精准地得到文件的最后修改时间。
