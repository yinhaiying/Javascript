...
a.age = 14;
```
那么使用遍历时，会遍历`name`和`age`属性。而不仅仅是a自身身上的`age`属性。因此，使用`for in`遍历时我们最好把原型上的属性区分开来，通过`hasOwnProperty`筛选出自身的属性进行遍历。
```javascript
    for (let key in source) {
      // 只遍历本身的属性
...
```
我们可以发现，所哟䣌deepClone共用的是同一个cache，也就是说这些在多个对象进行深拷贝的时候，他们之间的cache是会互相影响的。示例：
```javascript
  let a = {
    name:"hello",
  }     
...
```
我们发现在深拷贝对象b的时候，map中已经有值了。而事实上这些值不应该是b身上已经拷贝过的。这会导致问题。因此，我们不能让所有的深拷贝共用同一个缓存，而是让每一个深拷贝使用自己的属性。解决办法是：在调用函数时，每次都创建一个新的map，然后如果需要递归，就把这个map往下传。
```javascript
